...
const Pop = () => {
  ...
  const handleBubbleClick = async (id) => {
    playSound();
    ...
    // Configurar un nuevo temporizador para guardar automáticamente el puntaje después de 5 segundos de inactividad
    ...
    try {
      const response = await fetch('http://localhost:3000/api/logs', {
        ...
        body: JSON.stringify({
          action: 'bubble_click',
          username: userId,  // Asegúrate de tener el ID del usuario disponible
          timestamp: new Date(),
        }),
      });
  ...
  const resetGame = () => {
    ...
    setBubblesPressed(0);
    setScoreSaved(false);
    clearTimeout(timeoutRef.current); // Limpiar el temporizador al reiniciar el juego
  };
  ...
  useEffect(() => {
    const handleScroll = async () => {
      // Registrar el desplazamiento en MongoDB
      try {
        const response = await fetch('http://localhost:3000/api/logs', {
          method: 'POST',
          headers: {
        ...
        const data = await response.json();
        console.log('Scroll logged:', data);
      } catch (error) {
